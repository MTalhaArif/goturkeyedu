    ...
    <div style={{ backgroundColor: "#f8fafc", minHeight: "100vh", paddingTop: 40, paddingBottom: 80 }}>
      <div className="container">
        ...
        <div style={{ background: "linear-gradient(135deg, var(--secondary) 0%, #283593 100%)", borderRadius: 16, padding: "35px 48px", marginBottom: 36, color: "white", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 20 }}>
          <div>
            <h1 style={{ fontSize: 32, marginBottom: 8 }}>Program & University Search</h1>
            <p style={{ opacity: 0.82, fontSize: 15 }}>Search among 208 universities and nearly 45,000 degree programs in Türkiye</p>
          </div>
          <div style={{ display: "flex", gap: 30 }}>
            <div style={{ textAlign: "center" }}>
              <div style={{ fontSize: 40, fontWeight: 900, color: "var(--accent)" }}>{universities.length}</div>
              <div style={{ fontSize: 12, opacity: 0.8 }}>Universities</div>
            </div>
            <div style={{ textAlign: "center" }}>
              <div style={{ fontSize: 40, fontWeight: 900, color: "var(--accent)" }}>{filtered.reduce((s, u) => s + u.programs.length, 0)}</div>
              <div style={{ fontSize: 12, opacity: 0.8 }}>Programs</div>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "290px 1fr", gap: 28 }}>
          {/* FILTERS */}
          <div style={{ background: "white", borderRadius: 16, padding: 28, boxShadow: "0 4px 20px rgba(0,0,0,0.05)", alignSelf: "start", position: "sticky", top: 80 }}>
            <h3 style={{ color: "var(--secondary)", borderBottom: "2px solid var(--border)", paddingBottom: 14, marginBottom: 22, fontSize: 17 }}>🔍 Filters</h3>

            <div style={{ marginBottom: 18 }}>
              <label style={{ display: "block", fontWeight: 700, marginBottom: 7, fontSize: 13, color: "var(--secondary)", textTransform: "uppercase", letterSpacing: "0.5px" }}>Program / University</label>
              <input type="text" placeholder="Search programs or universities..." className="form-input" value={programSearch} onChange={e => setProgramSearch(e.target.value)} />
            </div>

            <div style={{ marginBottom: 18 }}>
              <label style={{ display: "block", fontWeight: 700, marginBottom: 7, fontSize: 13, color: "var(--secondary)", textTransform: "uppercase", letterSpacing: "0.5px" }}>Education Level</label>
            ...
            <div style={{ marginBottom: 18 }}>
              <label style={{ display: "block", fontWeight: 700, marginBottom: 7, fontSize: 13, color: "var(--secondary)", textTransform: "uppercase", letterSpacing: "0.5px" }}>Teaching Language</label>
            ...
            <div style={{ marginBottom: 18 }}>
              <label style={{ display: "block", fontWeight: 700, marginBottom: 7, fontSize: 13, color: "var(--secondary)", textTransform: "uppercase", letterSpacing: "0.5px" }}>City</label>
            ...
            <div style={{ marginBottom: 22 }}>
              <label style={{ display: "block", fontWeight: 700, marginBottom: 10, fontSize: 13, color: "var(--secondary)", textTransform: "uppercase", letterSpacing: "0.5px" }}>University Type</label>
              {[["", "All Types"], ["State", "State University"], ["Foundation", "Foundation (Private)"]].map(([val, label]) => (
            ...
            <div style={{ marginTop: 22, borderTop: "1px solid var(--border)", paddingTop: 18 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: "var(--secondary)", marginBottom: 10, textTransform: "uppercase", letterSpacing: "0.5px" }}>Quick City Search</div>
          ...
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <div>
                <h2 style={{ fontSize: 20, color: "var(--secondary)" }}>Search Results</h2>
                {(programSearch || selectedCity || selectedType) && (
                  <p style={{ fontSize: 13, color: "var(--text-muted)", marginTop: 4 }}>
                    {programSearch && `Program: "${programSearch}"`} {selectedCity && `| City: ${selectedCity}`} {selectedType && `| Type: ${selectedType}`}
                  </p>
                )}
              </div>
            ...
            {filtered.length === 0 ? (
              <div style={{ textAlign: "center", padding: "80px 40px", background: "white", borderRadius: 16 }}>
                ...
                <h3 style={{ color: "var(--secondary)", marginBottom: 10 }}>No results found</h3>
                <p style={{ color: "var(--text-muted)" }}>Try adjusting your search filters</p>
                <button className="btn-primary" style={{ marginTop: 20 }} onClick={() => { setSelectedCity(""); setSelectedType(""); setProgramSearch(""); }}>Clear Filters</button>
              </div>
            ) : (
                    ...
                    <div key={uni.id} style={{ background: "white", borderRadius: 16, overflow: "hidden", boxShadow: "0 4px 20px rgba(0,0,0,0.05)", border: isExpanded ? "1.5px solid var(--primary)" : "1.5px solid transparent", transition: "all 0.3s" }}>
                      ...
                      <div style={{ padding: "10px 26px", background: "rgba(26,35,126,0.03)", borderBottom: "1px solid var(--border)", fontSize: 12.5, color: "var(--text-muted)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        ...
                        <div style={{ display: "flex", gap: 12, flexShrink: 0 }}>
                          <a href={`https://${uni.website}`} target="_blank" rel="noreferrer" style={{ fontSize: 12, color: "var(--primary)", fontWeight: 600 }}>Visit Website ↗</a>
                        </div>
                      </div>
                        ...
                        {!isExpanded ? (
                          <div>
                            <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
                              ...
                              {programs.length > 8 && (
                                <button onClick={() => setExpandedUni(uni.id)} style={{ background: "rgba(224,60,49,0.08)", color: "var(--primary)", border: "none", padding: "4px 11px", borderRadius: 5, fontSize: 12.5, fontWeight: 700, cursor: "pointer" }}>+{programs.length - 8} more</button>
                              )}
                            </div>
                            <button className="btn-primary" style={{ marginTop: 14, fontSize: 13, padding: "8px 20px" }} onClick={() => setExpandedUni(uni.id)}>View All Programs & Apply</button>
                          </div>
                        ) : (
                          <div>
                            <h4 style={{ color: "var(--secondary)", marginBottom: 14, fontSize: 15 }}>All Programs ({programs.length})</h4>
                            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
                              <thead>
                                <tr style={{ background: "rgba(26,35,126,0.05)" }}>
                                  <th style={{ padding: "10px 14px", textAlign: "left", color: "var(--secondary)", fontWeight: 700, borderBottom: "2px solid var(--border)" }}>Program Name</th>
                                  <th style={{ padding: "10px 14px", textAlign: "left", color: "var(--secondary)", fontWeight: 700, borderBottom: "2px solid var(--border)" }}>Degree</th>
                                  <th style={{ padding: "10px 14px", textAlign: "left", color: "var(--secondary)", fontWeight: 700, borderBottom: "2px solid var(--border)" }}>Language</th>
                                  <th style={{ padding: "10px 14px", textAlign: "left", color: "var(--secondary)", fontWeight: 700, borderBottom: "2px solid var(--border)" }}>Action</th>
                                </tr>
                              </thead>
                              <tbody>
                                {programs.map((prog, i) => (
                                  <tr key={i} style={{ borderBottom: "1px solid var(--border)", background: i % 2 === 0 ? "white" : "rgba(26,35,126,0.015)" }}>
                                    ...
                                    <td style={{ padding: "12px 14px", color: "var(--text-muted)", fontSize: 13 }}>Bachelor's Degree</td>
                                    <td style={{ padding: "12px 14px" }}>
                                      <span style={{ background: "rgba(0,120,60,0.1)", color: "#006633", padding: "2px 9px", borderRadius: 4, fontSize: 12, fontWeight: 600 }}>
                                        {["Engineering","Computer","Electrical","Civil","Mechanical"].some(k => prog.includes(k)) ? "English" : "Turkish / English"}
                                      </span>
                                    </td>
                                    <td style={{ padding: "12px 14px" }}>
                            ...
                            <div style={{ marginTop: 16, display: "flex", gap: 12 }}>
                              ...
                              <button style={{ background: "none", border: "none", color: "var(--text-muted)", cursor: "pointer", fontSize: 13 }} onClick={() => setExpandedUni(null)}>Collapse ▲</button>
                            </div>
                          </div>
                        )}
                      </div>
